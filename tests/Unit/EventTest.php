...
class EventTest extends
     ...
     */
    public
    function testStatusChangedToApproved(){
        $this->assertCount(
            0,
            Event::all()
        );
        ...
        $this->assertEquals(
            $expectedStatus,
            $validatedEvent->approval_status
        );
    }
     ...
     */
    public
    function testListValidatedEvents(){
        $this->assertCount(
            0,
            Event::all()
        );
        ...
        $validatedEvent =
            $this->get('/api/validatedEvents/')->decodeResponseJson()[1];
